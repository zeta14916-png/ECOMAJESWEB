...
- `ecomajes/views/alertas.py` — Alertas module (pages `adm_mn_alertas`, `adm_msu_alertas`; ÁREA ADMINISTRATIVA, read-only). Replenishment alerts scoped to the current sede (`ctx["sede"]`, never all sedes) and material type. Lists active products that are Agotado (stock ≤ 0) or Stock bajo (stock_minimo > 0 and stock ≤ stock_minimo), with columns Código/Descripción/Categoría/Stock actual/Stock mínimo/Cantidad sugerida a pedir (= max(stock_minimo − stock, 0))/Estado. Reuses `db.list_catalog_products` only — no inventory/movement logic changed
