...
- `ecomajes/views/importar_productos.py` — Importar Productos module (page `ger_importar_productos`, GERENCIA-only; the single sanctioned nav addition). Uploads an **.xlsx** (legacy `.xls` intentionally not accepted — `xlrd` isn't installed), normalizes headers case-insensitively (aliases in `_COLUMN_ALIASES`: CODIGO/DESCRIPCION/UNIDAD/PRECIO DE VENTA SOLES/COSTO INICIAL SOLES/FAMILIA/CATEGORIA), dedups repeated Código in-file (`_dedupe`, keeps the fullest DESCRIPCION, reports skipped duplicates), previews the normalized rows, then calls `db.import_products`. **PRECIO/COSTO/FAMILIA are read but NOT persisted** (respects "do not modify prices"; products still appear in Precios via the existing LEFT JOIN). GERENCIA picks Sede destino + Tipo de material for NEW products; existing códigos update wherever they live. Summarizes Importados/Actualizados/Duplicados omitidos/Errores; best-effort `db.log_audit(AUDIT_IMPORT, ...)`
...
- `db.import_products(rows, sede, material_tipo)` — Excel importer upsert used by Importar Productos. Matches on globally-unique `codigo` (partial-unique index enables identity across sedes): existing codigo → UPDATE descripcion(fullest)/categoria/unidad (leaves nombre/stock); missing codigo → matched by `nombre` (=descripcion) within (sede, material_tipo); new → INSERT with nombre=descripcion (on a `(sede,material_tipo,nombre)` collision it appends ` (codigo)` to keep nombre unique). Per-row SAVEPOINT so one bad row can't abort the batch. `db.classify_categoria(descripcion)` auto-buckets into `IMPORT_CATEGORIES` (TUBO RECTANGULAR/REDONDO/CUADRADO, PLATINAS, ANGULOS, PLANCHAS, OTROS). Never touches `prices`
