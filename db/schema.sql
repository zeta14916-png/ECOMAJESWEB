...
--
-- FASE 2: Nuevas tablas y columnas
--

ALTER TABLE public.movements
    ADD COLUMN IF NOT EXISTS metodo_pago text
        CHECK (metodo_pago IS NULL OR metodo_pago = ANY (ARRAY[
            'efectivo'::text, 'yape'::text, 'plin'::text,
            'transferencia'::text, 'caja_chica'::text
        ]));

CREATE TABLE IF NOT EXISTS public.ingresos_adicionales (
    id          integer NOT NULL,
    fecha       date NOT NULL,
    descripcion text NOT NULL,
    monto       numeric(12,2) NOT NULL,
    ...
    usuario_rol text,
    created_at  timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT ingresos_adicionales_monto_check CHECK ((monto >= (0)::numeric))
);
CREATE SEQUENCE IF NOT EXISTS public.ingresos_adicionales_id_seq
    AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
ALTER SEQUENCE public.ingresos_adicionales_id_seq
    OWNED BY public.ingresos_adicionales.id;
ALTER TABLE ONLY public.ingresos_adicionales
    ALTER COLUMN id SET DEFAULT nextval('public.ingresos_adicionales_id_seq'::regclass);
ALTER TABLE ONLY public.ingresos_adicionales
    ADD CONSTRAINT ingresos_adicionales_pkey PRIMARY KEY (id);
CREATE INDEX IF NOT EXISTS idx_ingresos_ad_fecha_sede
    ON public.ingresos_adicionales USING btree (fecha, sede);
...
CREATE TABLE IF NOT EXISTS public.deudores (
    ...
    usuario_rol text,
    created_at  timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT deudores_monto_check  CHECK ((monto >= (0)::numeric)),
    CONSTRAINT deudores_estado_check CHECK ((estado = ANY (ARRAY['pendiente'::text, 'pagado'::text])))
);
CREATE SEQUENCE IF NOT EXISTS public.deudores_id_seq
    AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
ALTER SEQUENCE public.deudores_id_seq OWNED BY public.deudores.id;
ALTER TABLE ONLY public.deudores
    ALTER COLUMN id SET DEFAULT nextval('public.deudores_id_seq'::regclass);
ALTER TABLE ONLY public.deudores
    ADD CONSTRAINT deudores_pkey PRIMARY KEY (id);
CREATE INDEX IF NOT EXISTS idx_deudores_estado ON public.deudores USING btree (estado);
CREATE INDEX IF NOT EXISTS idx_deudores_sede   ON public.deudores USING btree (sede);

CREATE TABLE IF NOT EXISTS public.entrega_sobres (
    id           integer NOT NULL,
    fecha        date NOT NULL,
    ...
    usuario_rol  text,
    created_at   timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT entrega_sobres_monto_check CHECK ((monto >= (0)::numeric))
);
CREATE SEQUENCE IF NOT EXISTS public.entrega_sobres_id_seq
    AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
ALTER SEQUENCE public.entrega_sobres_id_seq OWNED BY public.entrega_sobres.id;
ALTER TABLE ONLY public.entrega_sobres
    ALTER COLUMN id SET DEFAULT nextval('public.entrega_sobres_id_seq'::regclass);
ALTER TABLE ONLY public.entrega_sobres
    ADD CONSTRAINT entrega_sobres_pkey PRIMARY KEY (id);
CREATE INDEX IF NOT EXISTS idx_entrega_sobres_fecha_sede
    ON public.entrega_sobres USING btree (fecha, sede);
...
ALTER TABLE ONLY public.compras
    ALTER COLUMN id SET DEFAULT nextval('public.compras_id_seq'::regclass);
ALTER TABLE ONLY public.compras
    ADD CONSTRAINT compras_pkey PRIMARY KEY (id);
CREATE INDEX IF NOT EXISTS idx_compras_sede       ON public.compras USING btree (sede);
CREATE INDEX IF NOT EXISTS idx_compras_fecha_sede ON public.compras USING btree (fecha, sede);
CREATE INDEX IF NOT EXISTS idx_compras_codigo     ON public.compras USING btree (codigo);
